...
sealed class ScafiIncarnation[T, P <: Position[P]] extends Incarnation[T, P] {
  ...
  override def createAction(
    ...
    if (param == "send") {
      ...
      if (spList.size > 1) {
        throw new IllegalStateException(
          "There are too many programs requiring a " + classOf[SendScafiMessage[T, P]].getName + " action: " + spList
        )
      }
  ...
  override def createCondition(
    ...
    if (!isScafiNode(node)) {
      throw new IllegalArgumentException(
        s"The node must be an instance of ${classOf[ScafiNode[_, _]]}"
  ...
  override def getProperty(node: Node[T], molecule: Molecule, propertyName: String) = {
    val imports = "import it.unibo.alchemist.model.implementations.nodes.{ScafiNode, SimpleNodeManager}"
    val context = "val self: NodeManager = new SimpleNodeManager[_](node)"
    ...
    if (propertyName == null || propertyName.trim.isEmpty) {
      ...
      toDouble(
        ScalaInterpreter(
          s"$imports; val value = " + target + ";" + propertyName
        )
      )
    }
  }
